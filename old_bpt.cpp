...
struct dat{
    char fi[65];
    int se;
    dat(){memset(fi, 0, sizeof(fi)); se = 0;}
    dat(const dat &b){
        memcpy(fi, b.fi, sizeof(fi));
        se = b.se;
    }
    ...
    bool is_eq(const dat& oth) const{return memcmp(fi, oth.fi, 64) == 0;}
    bool is_compare(const dat& oth) const{return memcmp(fi, oth.fi, 64) <= 0;}
    bool operator <(const dat& oth) const{
        int cur = memcmp(fi, oth.fi, 64);
        return cur < 0 || cur == 0 && se < oth.se;
    }
    bool operator <=(const dat& oth) const{
        int cur = memcmp(fi, oth.fi, 64);
        return cur < 0 || cur == 0 && se <= oth.se;
    ...
    bool operator ==(const dat& oth) const{
        return memcmp(fi, oth.fi, 64) == 0 && se == oth.se; 
    }
    bool operator !=(const dat& oth) const{
        return memcmp(fi, oth.fi, 64) != 0 || se != oth.se;
    }
...
public:
    FILE *a;
    bpt(const char *s, const char *t):A(s),C(t){
        a = A.f;
        if(A.empty) rt = 0, cnt = 0;
        else{
            A.read<int>(0, &rt);
            A.read<int>(4, &cnt);
        }
    ...
    int que(Node &cur, const T& x){
        int l = 0, r = cur.sz;
        while(l < r){
            int mid = (l + r) >> 1;
            if(x.is_compare(cur.a[mid])) r = mid;
            else l = mid + 1;
        }
        return l;
    }
